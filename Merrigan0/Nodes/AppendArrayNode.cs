using System;
using System.Collections.Generic;
using System.Diagnostics;

////namespace Merrigan0.Internal.Nodes {
    ////[Untested]
////    // When you have an array map and another map for the end of it
////    internal class AppendArrayNode : ArrayNode {
////        private ArrayNode arrayNode;
////        private Node map;

////        public override long Length { get { return arrayNode.Length + 1; } }

////        public AppendArrayNode(ArrayNode previous, Node item) { 
////            this.arrayNode = previous;
////            this.map = item;
////        }

////        /// <summary>
////        /// Copies the elements of the ICollection<T> to an Array, starting at a particular Array index.
////        /// </summary>
////        /// <param name="loadedItems">The one-dimensional Array that is the destination of the elements copied from ICollection<T>. The Array must have zero-based indexing.</param>
////        /// <param name="arrayIndex">The zero-based index in array at which copying begins.</param>
////        /// <exception cref="ArgumentNullException">array is null.</exception>
////        /// <exception cref="ArgumentOutOfRangeException">arrayIndex is less than 0.</exception>
////        public override void CopyTo(Node[] array, long arrayIndex) {
////            arrayNode.CopyTo(array, arrayIndex);
////            array[arrayIndex + arrayNode.Length] = map;
////        }

////        protected override Node GetCharacter(long i) {
////            if (i < arrayNode.Length) {
////                return arrayNode[i];
////            }
////            if (i > arrayNode.Length) {
////                throw new IndexOutOfRangeException();
////            }
////            return map;
////        }
////    }
////}
