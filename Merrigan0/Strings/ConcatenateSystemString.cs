...
namespace Merrigan0.StringsInternal {
    [Untested]
    internal class ConcatenateSystemString : String {
        private string s1;
        private string s2;

        public override long Length { get { return s1.Length + s2.Length; } }

        public ConcatenateSystemString(string s1, string s2)
        { 
        ...
        public override void CopyTo(char[] array, int arrayIndex) {
            s1.CopyTo(0, array, arrayIndex, s1.Length);
            s2.CopyTo(0, array, arrayIndex + (int)s1.Length, s2.Length);
        }
